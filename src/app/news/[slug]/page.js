...
// Dynamically import detail and related sections
const NewsDetailSection = dynamic(() => import("@/components/features/news/NewsDetailSection"), { ssr: true });

const RelatedSection = dynamic(() => import("@/components/features/news/RelatedSection"), { ssr: true });
    ...
    <>
      {banner?.enable__disable_news_detail_banner && (
        <InnerHero
          ...
          desktopImage={banner.news_detail_desktop_image?.url || "/images/placeholder.jpg"}
      ...
      {post.related_posts && post.related_posts.length > 0 && <RelatedSection data={post} />}
    </>
  );
}
