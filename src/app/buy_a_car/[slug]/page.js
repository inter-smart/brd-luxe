...
// Dynamically import all sections (product detail pages benefit most from this)
const ProductDetailSection = dynamic(() => import("@/components/features/buy/ProductDetailSection"), { ssr: true });

const KeyHighlightSection = dynamic(() => import("@/components/features/buy/KeyHighlightSection"), { ssr: true });

const EnquirySection = dynamic(() => import("@/components/features/contact/EnquirySection"), { ssr: true });

async function getPageData() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/wp-json/custom/v1/buy`, { next: { revalidate: 60 } });
...
export async function generateMetadata({ params }) {
  ...
  const cars = Array.isArray(data?.listingpagedata?.cars_data) ? data.listingpagedata.cars_data : [];
