...
export default function FiltersData({ data, whatsapp }) {
  ...
  if (appliedFilters.brand) {
    filteredCars = filteredCars.filter((car) => car.brand?.some((b) => b.toLowerCase().replace(/\s+/g, "-") === appliedFilters.brand));
  }
  if (appliedFilters.model) {
    filteredCars = filteredCars.filter((car) => car.model?.some((m) => m.toLowerCase().replace(/\s+/g, "-") === appliedFilters.model));
  }
    ...
    <>
      <div className="mb-[25px] lg:mb-[10px] 3xl:mb-[30px] flex max-md:flex-wrap items-center">
        <div className="w-full md:w-1/3">
          <Heading as="h1" size="heading1" className="text-white max-sm:mb-[15px] max-md:text-center md:text-left">
            {cars_section?.title}
          </Heading>
        </div>
      ...
      {!isMobile ? (
        ...
        <p className="text-center text-white text-[14px] lg:text-[16px] 2xl:text-[18px] font-base1 py-[30px]">No cars found matching your filters.</p>
      )}
    </>
  );
}
