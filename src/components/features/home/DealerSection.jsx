...
export default function DealerSection({ data }) {
  ...
  return (
    dealers_section?.enable__disable_dealers_section?
        ...
        <div className="container">
          <div className="[&>*]:max-md:text-center [&>*]:w-full [&>*]:md:w-1/3 md:space-x-[20px] lg:space-x-[35px] 2xl:space-x-[45px] 3xl:space-x-[60px] flex items-center max-md:flex-col">
            ...
            <motion.div {...fadeUp()}>
              ...
              <StyledLink href={dealers_section?.button_url?.url} target={dealers_section?.button_url?.target}>{dealers_section?.button_title}</StyledLink>
              )}
            </motion.div>
          </div>
        </div>
