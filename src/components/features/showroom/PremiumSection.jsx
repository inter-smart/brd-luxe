...
export default function PremiumSection({ data }) {
  const premium_showroom_section = data?.showroom_acf?.premium_showroom_section;

  return premium_showroom_section?.enable__disable_premium_section === true ? (
    <section
      className="w-full h-full min-h-[300px] sm:min-h-[450px] xl:min-h-[550px] 2xl:min-h-[650px] 3xl:min-h-[750px] relative 
    ...
    >
      <div
        className="w-full h-full sm:max-w-[75%] absolute top-0 left-0 after:absolute after:bottom-0 after:right-0 
                after:content-[''] after:h-full after:w-[450px] after:z-10
                after:bg-[linear-gradient(270deg,#000_0%,rgba(0,0,0,0)_100%)] "
      >
        <Image
          src={premium_showroom_section?.image?.url}
          alt={premium_showroom_section?.image?.alt}
          width="1400"
          height="890"
          placeholder="blur"
      ...
      {/* title */}
      <div
        className="text-[35px] sm:text-[45px] lg:text-[50px] xl:text-[65px] 2xl:text-[80px] 3xl:text-[100px] font-bold font-base1 
                uppercase absolute top-0 bottom-0 right-0 md:right-[50px] z-20 m-auto h-auto w-auto
                        ...
                        [background-image:linear-gradient(270deg,#141414_0%,#252525_51.92%,#141414_100%)]"
        style={{ writingMode: "sideways-lr" }}
      >
        {premium_showroom_section?.side_text}
      </div>
      {/* breadcrumb */}
      <div className="container absolute top-[40px] left-0 z-20 right-0 m-auto">
        <BreadCrumb
          items={[
            { label: "Home", href: "/" },
            { label: data.title, href: "/showroom" },
          ]}
        />
      </div>
      <div className="container relative z-10 ">
        <div className="w-full sm:max-w-[350px] xl:max-w-[400px] 2xl:max-w-[500px] 3xl:max-w-[700px] ml-auto relative ">
          <div className="max-w-[275px] xl:max-w-[350px] 2xl:max-w-[420px] 3xl:max-w-[550px]">
            <div
              className="text-[22px] leading-[22px] 
              ...
              dangerouslySetInnerHTML={{
                __html: premium_showroom_section?.title || "",
              }}
            />
            <p className="text-[10px] xl:text-[13px] 2xl:text-[16px] 3xl:text-[20px] !font-base2 text-white font-extralight">
